...
class MyTopo( Topo ):
    ...
    def build( self ):
        ...
        h1 = self.addHost( 'h1', mac="00:00:00:00:00:01", ip="10.0.0.1/24", defaultRoute='via 10.0.0.10')
        h2 = self.addHost( 'h2', mac="00:00:00:00:00:02", ip="10.0.0.2/24", defaultRoute='via 10.0.0.10')
        ...
        s4 = self.addSwitch( 's4', cls=OVSKernelSwitch, protocols='OpenFlow13' ) # Switch Client-1

        h3 = self.addHost( 'h3', mac="00:00:00:00:00:03", ip="172.16.0.1/24", defaultRoute='via 172.16.0.10')
        h4 = self.addHost( 'h4', mac="00:00:00:00:00:04", ip="172.16.0.2/24", defaultRoute='via 172.16.0.10')
        h5 = self.addHost( 'h5', mac="00:00:00:00:00:05", ip="172.16.0.3/24", defaultRoute='via 172.16.0.10')
        h6 = self.addHost( 'h6', mac="00:00:00:00:00:06", ip="172.16.0.4/24", defaultRoute='via 172.16.0.10')

        s5 = self.addSwitch( 's5', cls=OVSKernelSwitch, protocols='OpenFlow13' ) # Switch Client-2

        h7 = self.addHost( 'h7', mac="00:00:00:00:00:07", ip="172.16.10.1/24", defaultRoute='via 172.16.10.10')
        h8 = self.addHost( 'h8', mac="00:00:00:00:00:08", ip="172.16.10.2/24", defaultRoute='via 172.16.10.10')
        h9 = self.addHost( 'h9', mac="00:00:00:00:00:09", ip="172.16.10.3/24", defaultRoute='via 172.16.10.10')
        h10 = self.addHost( 'h10', mac="00:00:00:00:00:0A", ip="172.16.10.4/24", defaultRoute='via 172.16.10.10')
        h11 = self.addHost( 'h11', mac="00:00:00:00:00:0B", ip="172.16.10.5/24", defaultRoute='via 172.16.10.10')
        h12 = self.addHost( 'h12', mac="00:00:00:00:00:0C", ip="172.16.10.6/24", defaultRoute='via 172.16.10.10')
...
def deleteFile(host):
    host.cmdPrint('rm -f *.html;rm -f *.txt;rm -f *.zip;')
    host.cmdPrint("ls -alh;")
    sleep(10)
    return
...
def do_tcp(host,target,port):
    host.cmdPrint('python3 ../localhost/tcpsocket-client.py {} {} search ring;'.format(target,port))
    host.cmdPrint('python3 ../localhost/tcpsocket-client.py {} {} search text1;'.format(target,port))
    host.cmdPrint('python3 ../localhost/tcpsocket-client.py {} {} search text2;'.format(target,port))
    host.cmdPrint('python3 ../localhost/tcpsocket-client.py {} {} search text3;'.format(target,port))
    sleep(5)
    return

def do_tftp_get(host,target,port):
    # collection = ['01-1.txt','02-1.txt','05-1.txt','10-1.txt','15-1.txt','20-1.txt']
    if target == '10.0.0.1':
        collection = ['01-1.txt','02-1.txt','05-1.txt','10-1.txt','15-1.txt','20-1.txt']
        for i in collection:
            host.cmdPrint('date;python3 ../localhost/client.py {} {} -g {} ;date;'.format(target,port,i))
    elif target == '10.0.0.2':
        collection = ['01-2.txt','02-2.txt','05-2.txt','10-2.txt','15-2.txt','20-2.txt']
        for i in collection:
            host.cmdPrint('date;python3 ../localhost/client.py {} {} -g {} ;date;'.format(target,port,i))
    else:
        return

    # host.cmdPrint('python3 ../localhost/client.py {} {} -g 01.txt ;'.format(target,port))
    # host.cmdPrint('python3 ../localhost/client.py {} {} -g 05.txt ;'.format(target,port))
    # host.cmdPrint('python3 ../localhost/client.py {} {} -g 10.txt ;'.format(target,port))
    # host.cmdPrint('python3 ../localhost/client.py {} {} -g 15.txt ;'.format(target,port))
    # host.cmdPrint('python3 ../localhost/client.py {} {} -g 20.txt ;'.format(target,port))
    host.cmdPrint("ls -alh;")
    sleep(5)
    return

def do_tftp_put(host,target,port,folder):
    host.cmdPrint('python3 ../localhost/client.py {} {} -p /home/guest/flowmanager/topo/{}/text1.txt ;'.format(target,port,folder))
    host.cmdPrint('python3 ../localhost/client.py {} {} -p /home/guest/flowmanager/topo/{}/text2.txt ;'.format(target,port,folder))
    host.cmdPrint('python3 ../localhost/client.py {} {} -p /home/guest/flowmanager/topo/{}/text3.txt ;'.format(target,port,folder))
    host.cmdPrint('python3 ../localhost/client.py {} {} -p /home/guest/flowmanager/topo/{}/text4.txt ;'.format(target,port,folder))
    host.cmdPrint('python3 ../localhost/client.py {} {} -p /home/guest/flowmanager/topo/{}/index.html ;'.format(target,port,folder))
    host.cmdPrint('python3 ../localhost/client.py {} {} -p /home/guest/flowmanager/topo/{}/txt.zip ;'.format(target,port,folder))

    # host.cmdPrint('date;python3 ../client.py {} {} -p text2.txt ;date;'.format(target,port))
    # host.cmdPrint('date;python3 ../client.py {} {} -p text3.txt ;date;'.format(target,port))
    # host.cmdPrint('date;python3 ../client.py {} {} -p text4.txt ;date;'.format(target,port))
    # sleep(5)
    return
...
def scapy(host,target,port):
    # print("None")
    # return
    host.cmdPrint('date;python3 ../attackscapy.py 5 "{}:{}" "1-5" true;date;'.format(target,port))
    host.cmdPrint('date;python3 ../attackscapy.py 5 "{}:{}" "1-5" false;date;'.format(target,port))
    host.cmdPrint('date;python3 ../attackscapy.py 5 "{}:{}" "2-5" true;date;'.format(target,port))
    host.cmdPrint('date;python3 ../attackscapy.py 5 "{}:{}" "2-5" false;date;'.format(target,port))
    host.cmdPrint('date;python3 ../attackscapy.py 5 "{}:{}" "3-5" true;date;'.format(target,port))
    host.cmdPrint('date;python3 ../attackscapy.py 5 "{}:{}" "3-5" false;date;'.format(target,port))
    return
...
def startNetwork():
    ...
    c0.cmdPrint('tcpdump -vv -XX -tttt -nne -i s2-eth2 -w Mininet_Traffic_s2e2.pcap &')
    c0.cmdPrint('tcpdump -vv -XX -tttt -nne -i s4-eth1 -w Mininet_Traffic_s4e1.pcap &')
    c0.cmdPrint('tcpdump -vv -XX -tttt -nne -i s5-eth1 -w Mininet_Traffic_s5e1.pcap &')
    sleep(5)
    ...
    for i in range(5):
        ...
        for j in range(5):
            ...
            if j < 4:
                ...
                src.cmdPrint("timeout 30s hping3 -1 -d 999 10.0.0.1 --flood")
                src.cmdPrint("timeout 30s hping3 -1 -d 999 {} --flood &".format(dst))
                src.cmdPrint("timeout 30s hping3 -2 -p 1 -d 512 10.0.0.1 --flood")
                src.cmdPrint("timeout 30s hping3 -2 -p 1 -d 512 {} --flood &".format(dst))
                src.cmdPrint("timeout 30s hping3 -S -p 443 -d 999 -w 64 10.0.0.1 --flood ")
                src.cmdPrint("timeout 30s hping3 -S -p 443 -d 999 -w 64 {} --flood &".format(dst))
                ...
                src.cmdPrint("timeout 30s hping3 -1 -d 999 10.0.0.2 --flood")
                src.cmdPrint("timeout 30s hping3 -1 -d 999 {} --flood &".format(dst))
                src.cmdPrint("timeout 30s hping3 -2 -p 1 -d 512 10.0.0.2 --flood")
                src.cmdPrint("timeout 30s hping3 -2 -p 1 -d 512 {} --flood &".format(dst))
                src.cmdPrint("timeout 30s hping3 -S -p 443 -d 999 -w 64 10.0.0.2 --flood")
                src.cmdPrint("timeout 30s hping3 -S -p 443 -d 999 -w 64 {} --flood &".format(dst))
                ...
                src.cmdPrint("timeout 10s hping3 -1 -d 999 10.0.0.1 --flood")
                src.cmdPrint("timeout 10s hping3 -1 -d 999 {} --flood &".format(dst))
                src.cmdPrint("timeout 10s hping3 -2 -p 1 -d 512 10.0.0.1 --flood")
                src.cmdPrint("timeout 10s hping3 -2 -p 1 -d 512 {} --flood &".format(dst))
                src.cmdPrint("timeout 10s hping3 -S -p 443 -d 999 -w 64 10.0.0.1 --flood")
                src.cmdPrint("timeout 10s hping3 -S -p 443 -d 999 -w 64 {} --flood &".format(dst))
                do_tcp(src,'10.0.0.1',8888)
                # sleep(2)
                # do_tftp_get(src,'10.0.0.1',69)
                ...
                src.cmdPrint("timeout 10s hping3 -1 -d 999 10.0.0.2 --flood")
                src.cmdPrint("timeout 10s hping3 -1 -d 999 {} --flood &".format(dst))
                src.cmdPrint("timeout 10s hping3 -2 -p 1 -d 512 10.0.0.2 --flood")
                src.cmdPrint("timeout 10s hping3 -2 -p 1 -d 512 {} --flood &".format(dst))
                src.cmdPrint("timeout 10s hping3 -S -p 443 -d 999 -w 64 10.0.0.2 --flood")
                src.cmdPrint("timeout 10s hping3 -S -p 443 -d 999 -w 64 {} --flood & ".format(dst))
                do_tcp(src,'10.0.0.2',9999)
                sleep(2)
                # do_tftp_get(src,'10.0.0.2',70)
